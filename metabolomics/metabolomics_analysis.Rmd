...
path = "/Users/maureencarey/local_documents/work/TUMI_efforts/microbiome_course_2021"
...
Next, let's see look at a all of the metabolites at once. For example, we'd expect to see a similar range of metabolite abundances in all sample groups. A histogram would be useful for this purpose - can we generate it?
...
Random forest is an ensemble machine learning approach used to classify samples into particular groups (i.e. a classifier) or to predict a variable (i.e. regressor). For a classifier, many decision trees are build from subsets of data and internally validated with respect to the remaining dataset. With this approach, variables can be ranked by their effect of the classifer accuracy.

![Random Forest analysis from Carey et al 2018](./RF.jpg)
...
## Partial least squares discriminant analysis - NOT DONE
...
## PLS-DA function
plsda.res = plsda(data_for_plsda_wo_grp, data_for_plsda$group, ncomp = 2, scale = TRUE,
                   logratio="CLR") # where ncomp is the number of components wanted
```
...
set.seed(2021) # for reproducibility here, only when the `cpus' argument is not used
